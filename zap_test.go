...
func (uu users) MarshalArray(encoder logy.ArrayEncoder) error {
	var err error
	for i := range uu {
		err = encoder.AppendObject(uu[i])
	}
	return err
}
...
func (u *user) MarshalObject(encoder logy.ObjectEncoder) error {
	encoder.AddString("name", u.Name)
	encoder.AddString("email", u.Email)
	encoder.AddTime("created_at", u.CreatedAt)
	return nil
}
...
func (u *user) String() string {
	...
	builder.WriteString(strconv.FormatInt(u.CreatedAt.UnixNano(), 10))
	builder.WriteString("}")
